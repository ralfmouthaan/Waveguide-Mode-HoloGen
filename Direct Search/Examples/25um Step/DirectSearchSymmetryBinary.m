...
function [RetVal] = DirectSearchSymmetryBinary(Illum, Target, ReplayMask)
    ...
    if sum(sum(abs(TargetTop - TargetBottom))) < 1
        UpDownSymmetry = 1;
    elseif sum(sum(abs(TargetTop + TargetBottom))) < 1
        UpDownSymmetry = -1;
    else
    ...
    if sum(sum(abs(TargetLeft - TargetRight))) < 1
        LeftRightSymmetry = 1;
    elseif sum(sum(abs(TargetLeft + TargetRight))) < 1
        LeftRightSymmetry = -1;
    else
        error('HoloAnneal: No axis of symmetry');
    end
