...
function [Holo] = DirectSearchSymmetryMulti(Illum, Target, ReplayMask)
    ...
    Holo = Illum.*exp(1i*2*pi*rand(size(Illum)));
    Replay = fft2(Holo);
    ...
    if abs(sum(sum(TargetTop - TargetBottom))) < 1
        UpDownSymmetry = 1;
    elseif abs(sum(sum(TargetTop + TargetBottom))) < 1
        UpDownSymmetry = -1;
    else
        error('HoloAnneal: No axis of symmetry');
    end
    ...
    if abs(sum(sum(TargetLeft - TargetRight))) < 1
        LeftRightSymmetry = 1;
    elseif abs(sum(sum(TargetLeft + TargetRight))) < 1
        LeftRightSymmetry = -1;
    else
        error('HoloAnneal: No axis of symmetry');
    end
    ...
    if bolOutputToFile == true
        ...
        fid_stddev = fopen('Std Dev.txt', 'w+');
    end
    
    while IterNo < 3e6 %|| (maxglobalc < 0.04)
        ...
        NewPixel = Holo(m,n)*exp(1i*2*pi*rand);
        ...
        if (localc^gamma*globalc > maxlocalc^gamma*maxglobalc)
            ...
            if IterNo - LastDisplayed > 1000
                ...
                if bolOutputToFile == true
                    ...
                    HoloVec = Holo(:);
                    HoloVec = angle(HoloVec);
                    HoloVec = HoloVec(HoloVec < 0);
                    fprintf(fid_stddev, '%d\t%f\n', IterNo, std(HoloVec));
                end
